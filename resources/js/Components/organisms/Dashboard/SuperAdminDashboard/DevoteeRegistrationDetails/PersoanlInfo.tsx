...
export default function PersonalInfo({ masterData, handleNext }: PersonalInfoProps) {
  const form = useForm({
    initialValues: {
      ...
      dob:masterData?.User?.dob?.toString() || '',
    ...
    <div>
      <Grid py={10}>
        <Grid.Col span={{ base: 12, md: 6, lg: 3 }}>
          <TextInput label="Name (नाम)"  
          {...form.getInputProps('name')}
          value={form.values.name}
          />
        </Grid.Col>
        <Grid.Col span={{ base: 12, md: 6, lg: 3 }}>
          <TextInput label="Initiated Name"  
            {...form.getInputProps('Initiated_name')}
            value={form.values.Initiated_name}
          />
        </Grid.Col>
        <Grid.Col span={{ base: 12, md: 6, lg: 3 }}>
          <TextInput label="Email Address (ईमेल आईडी)"
            {...form.getInputProps('email')}
            value={form.values.email}
          />
        </Grid.Col>
        <Grid.Col span={{ base: 12, md: 6, lg: 3 }}>
          <TextInput label="Mobile Number (मोबाइल नंबर)" 
