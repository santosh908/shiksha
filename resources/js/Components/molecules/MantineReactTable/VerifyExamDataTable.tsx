...
import { MantineReactTable, useMantineReactTable, type MRT_Row } from 'mantine-react-table';
...
const VerifyExamDataTable = <T,>({
    ...
    return rows.map((row) => {
      ...
      for (const key in row) {
        if (allowedKeys.includes(key)) {
          //@ts-ignore
          flattenedRow[key] = typeof row[key] === 'object' && row[key] !== null ? JSON.stringify(row[key]) : row[key];
        }
      }
      return flattenedRow;
    });
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <Box className="space-y-4 w-full">
        ...
        <div className="flex gap-3 flex-wrap">
          <Button disabled={filteredData.length === 0} onClick={handleExportAllRows} variant="outline">
            Export All Rows
          </Button>
          <Button
  ...
  return (
    <div className="">
      ...
      {/* Custom Pagination */}
      <Pagination total={totalPages} value={currentPage} onChange={(page: any) => setCurrentPage(page)} />
    </div>
  );
};

export default VerifyExamDataTable;
